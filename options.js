...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('add-comment-template-btn').addEventListener('click', openAddCommentForm);
  document.getElementById('comment-form-save-btn').addEventListener('click', saveCommentForm);
  document.getElementById('comment-form-cancel-btn').addEventListener('click', cancelCommentForm);
});

// ---- Completion comment template management ----

function loadCompletionTemplates(callback) {
  chrome.storage.local.get('completionTemplates', function (result) {
    callback(result.completionTemplates || []);
  });
}

function saveCompletionTemplates(templates) {
  chrome.storage.local.set({ 'completionTemplates': templates });
}

function renderCompletionTemplates() {
  loadCompletionTemplates(function (templates) {
    var container = document.getElementById('comment-template-list');
    container.innerHTML = '';
    ...
    container.querySelectorAll('button[data-action="edit"]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        openEditCommentForm(parseInt(btn.dataset.index));
      });
    });

    container.querySelectorAll('button[data-action="delete"]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        deleteCompletionTemplate(parseInt(btn.dataset.index));
      });
    });
  });
}
...
function openAddCommentForm() {
  document.getElementById('comment-form-edit-id').value = '';
  document.getElementById('comment-form-name').value = '';
  document.getElementById('comment-form-body').value = '';
  document.getElementById('comment-template-form').style.display = 'block';
  document.getElementById('comment-form-name').focus();
}

function openEditCommentForm(index) {
  loadCompletionTemplates(function (templates) {
    var t = templates[index];
    document.getElementById('comment-form-edit-id').value = String(index);
    document.getElementById('comment-form-name').value = t.name;
    document.getElementById('comment-form-body').value = t.body;
    document.getElementById('comment-template-form').style.display = 'block';
    document.getElementById('comment-form-name').focus();
  });
}
...
function saveCommentForm() {
  var name = document.getElementById('comment-form-name').value.trim();
  var body = document.getElementById('comment-form-body').value;
  var editId = document.getElementById('comment-form-edit-id').value;
  ...
  loadCompletionTemplates(function (templates) {
    ...
    saveCompletionTemplates(templates);
    document.getElementById('comment-template-form').style.display = 'none';
    renderCompletionTemplates();
  });
}
...
function deleteCompletionTemplate(index) {
  loadCompletionTemplates(function (templates) {
    templates.splice(index, 1);
    saveCompletionTemplates(templates);
    renderCompletionTemplates();
